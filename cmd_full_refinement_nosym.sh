#!/bin/bash
if [ $# -lt 3 ]; then
	echo "Usage: $0 <full_map_file> <resolution> <weight>"
	echo "Eg.: $0 1.mrc 3.3 35"
...
cp /gscratch/baker/binchen/EM-data_challenge/xmls/localrlx_2_nosym.xml localrlx.xml 
sed "s/resx/$2/;s/weight/$3/" flags_template >flags
if [ ! -s fsc_top200/top10_fsc_fullmap.txt ]; then
	...
	cat top50_molpro.txt | awk '{print $1;}' | sed 's/A_molprob.log/.pdb/' | sed 's/^/cal_fsc.sh /' | sed "s|$| ../$1 $2|" >fsc_top200/cal_fsc.sh
	cd fsc_top200
...
#   modify_sym_def.pl symm_def.txt
  ...
  if [ ! -f run.lock ]; then
    touch run.lock
    nohup /gscratch/baker/binchen/bin/rosetta_scripts.static.linuxgccrelease @flags -database /gscratch/dimaio/dimaio/Rosetta/database -in:file:s $pdb\.pdb -mute all &
    sleep 1	
  fi
  cd ..
 done;
...
for path in `cat fsc_top200/top10_fsc_fullmap.txt | sed 's/.pdb//'`;do
  cd $path
  for pdb in `ls ${path}_INPUT_*[^A].pdb`; do fsc=`head -n1 $pdb | awk '{print $NF;}'`; echo $pdb $fsc >>../all_fsc_refined_fullmap.txt; done;
  cd ..
done;
